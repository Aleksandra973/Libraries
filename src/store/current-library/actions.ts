...
import {CurrentLibraryResponse, CurrentLibrary} from './state';
import axios from 'axios';

const actions: ActionTree<CurrentLibrary, StateInterface> = {
  async getLibrary ({commit}, id) {
    try {
      let response = await axios.get<CurrentLibraryResponse> ('http://localhost:3000/libraries',
        {params: {_id: id}});
      let currentLibrariesResponse: CurrentLibraryResponse = response.data
      ...
      let currentLibrary: CurrentLibrary = {
        ...
        phone: currentLibrariesResponse.data.general.contacts.phones[0].value,
        image: currentLibrariesResponse.data.general.image.url,
      }
      commit('setLibraries', currentLibrary);
    } catch (e){
      console.log(e)
      return e;
    }
